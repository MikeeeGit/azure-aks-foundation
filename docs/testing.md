...
terraform test -test-directory=tests/disposable-lab -var-file=config/global.tfvars -var-file=config/uks/pprd/pprd.tfvars -var-file=examples/disposable-lab/clusters.tfvars
```
...
The disposable lab test checks both two-node system pools, explicit mixed scheduling, zero user pools, Free tier, unchanged host encryption and upgrade surge. It does not establish live Azure allocation or application availability.
