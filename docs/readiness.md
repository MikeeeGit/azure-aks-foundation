...
The maintained Gateway API sample uses platform-owned Envoy HTTPS LoadBalancers at `.21` and application-owned ClusterIP Services. The retained direct-Service alternative uses `.20`/HTTP without a controller. Choose the profile explicitly and align private `ingress_private_ip` metadata with its real frontend. Desired `ingress_handoff` IPs are metadata until Kubernetes/Azure allocate and expose them. Do not route gateway traffic to overlay pod CIDRs or the private API endpoint. See the [first sandbox runbook](https://github.com/MikeeeGit/terraform-delivery-templates/blob/main/docs/azure/sandbox-deployment.md) for the complete handoff and real Azure evidence gates.
